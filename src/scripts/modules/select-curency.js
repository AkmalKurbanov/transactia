...
function hideSubmenus() {
  document
    .querySelectorAll('.cselect__submenu.show')
...
/**************************************************************
 * 4. RENDER SELECTED (КЛЮЧЕВАЯ ТОЧКА)
 **************************************************************/
function renderSelected(select, option, forced = null) {
  ...
  const values = getValues(option);
  const code = forced || values?.[0]?.code || option.dataset.code;
  ...
  document.querySelectorAll('.cselect-js').forEach(select => {
    ...
    input.addEventListener('input', () => {
      ...
      ul.querySelectorAll('.cselect__option').forEach(option => {
        const label =
          option.querySelector('.cselect__label')?.textContent.toLowerCase() ||
          '';
        const code = option.dataset.code?.toLowerCase() || '';
        option.style.display =
          label.includes(value) || code.includes(value) ? '' : 'none';
      });
    });
  });
}
...
function syncCselectWidths() {
  document.querySelectorAll('.cselect-js').forEach(select => {
    ...
    if (dropdown) dropdown.style.width = width;
...
document.addEventListener('click', () => {
  requestAnimationFrame(syncCselectWidths);
});
